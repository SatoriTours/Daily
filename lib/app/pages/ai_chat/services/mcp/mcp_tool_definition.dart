...
class MCPToolRegistry {
  ...
  static final List<MCPToolDefinition> tools = [
    ...
    const MCPToolDefinition(
      ...
      parameters: {
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的日记数量，默认为1，最大为10', defaultValue: 1),
      },
    ),
    const MCPToolDefinition(
      ...
      parameters: {
        'date': MCPParameterDefinition(type: 'string', description: '日期，格式为YYYY-MM-DD或相对日期如today,yesterday'),
      },
      required: ['date'],
    ),
    const MCPToolDefinition(
      ...
      parameters: {
        'keyword': MCPParameterDefinition(type: 'string', description: '搜索关键词，多个关键词用逗号分隔'),
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的最大数量，默认为20', defaultValue: 20),
      },
      required: ['keyword'],
    ),
    const MCPToolDefinition(
      ...
      parameters: {
        ...
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的最大数量，默认为10', defaultValue: 10),
      },
      required: ['tag'],
    ),
    const MCPToolDefinition(name: 'get_diary_count', description: '获取日记的总数量。', parameters: {}),
    ...
    const MCPToolDefinition(
      ...
      parameters: {
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的文章数量，默认为5，最大为20', defaultValue: 5),
      },
    ),
    const MCPToolDefinition(
    ...
    const MCPToolDefinition(
      ...
      parameters: {'limit': MCPParameterDefinition(type: 'integer', description: '返回的最大数量，默认为10', defaultValue: 10)},
    ),
    const MCPToolDefinition(name: 'get_article_count', description: '获取文章的总数量。', parameters: {}),
    ...
    const MCPToolDefinition(
      ...
      parameters: {
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的书籍数量，默认为5，最大为20', defaultValue: 5),
      },
    ),
    const MCPToolDefinition(
      name: 'search_books',
      ...
      parameters: {
        'keyword': MCPParameterDefinition(type: 'string', description: '搜索关键词，多个关键词用逗号分隔'),
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的最大数量，默认为15', defaultValue: 15),
      },
      required: ['keyword'],
    ),
    const MCPToolDefinition(
    ...
    const MCPToolDefinition(
      ...
      parameters: {'book_id': MCPParameterDefinition(type: 'integer', description: '书籍ID')},
      required: ['book_id'],
    ),
    const MCPToolDefinition(name: 'get_book_count', description: '获取书籍的总数量。', parameters: {}),

    // 综合统计工具
    const MCPToolDefinition(name: 'get_statistics', description: '获取应用的综合统计信息。返回文章、日记、书籍的数量统计。', parameters: {}),
  ];

  static List<Map<String, dynamic>> getFunctionSchemas() => tools.map((tool) => tool.toFunctionSchema()).toList();
    ...
    for (final tool in tools) {
      ...
      if (tool.parameters.isNotEmpty) {
        ...
        for (final entry in tool.parameters.entries) {
          ...
          buffer.writeln('- **${entry.key}** $requiredMark: ${entry.value.description}');
        }
      }
      buffer.writeln();
    }
