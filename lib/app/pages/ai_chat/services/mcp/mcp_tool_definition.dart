...
返回最近创建的 N 条日记。''',
      parameters: {
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的日记数量，默认为 1，最大为 10', defaultValue: 1),
      },
    ),
    const MCPToolDefinition(
...
支持相对日期（today, yesterday）和具体日期。''',
      parameters: {
        'date': MCPParameterDefinition(type: 'string', description: '日期，格式为 YYYY-MM-DD 或相对日期如 today, yesterday'),
      },
      required: ['date'],
    ),
    const MCPToolDefinition(
...
按创建时间倒序返回匹配的日记。''',
      parameters: {
        ...
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的最大数量，默认为 20', defaultValue: 20),
      },
      required: ['keyword'],
    ),
    const MCPToolDefinition(
...
用于回答如"工作标签的日记"、"标记为重要的日记"等问题。''',
      parameters: {
        ...
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的最大数量，默认为 10', defaultValue: 10),
      },
      required: ['tag'],
    ),
    const MCPToolDefinition(
...
返回最近创建的 N 篇文章。''',
      parameters: {
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的文章数量，默认为 5，最大为 20', defaultValue: 5),
      },
    ),
    const MCPToolDefinition(
...
用于回答如"最近在读什么书"、"最新添加的书"等问题。''',
      parameters: {
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的书籍数量，默认为 5，最大为 20', defaultValue: 5),
      },
    ),
    const MCPToolDefinition(
      name: 'search_books',
...
用于回答如"有没有《xxx》这本书"、"xxx 作者的书"、"关于投资的书"等问题。''',
      parameters: {
        'keyword': MCPParameterDefinition(type: 'string', description: '搜索关键词（匹配书名、作者或分类），多个关键词用逗号分隔'),
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的最大数量，默认为 15', defaultValue: 15),
      },
      required: ['keyword'],
    ),
    const MCPToolDefinition(
...
当用户询问某个主题相关的内容时，也应该搜索读书笔记。''',
      parameters: {
        'keyword': MCPParameterDefinition(type: 'string', description: '搜索关键词（匹配笔记标题或内容），多个关键词用逗号分隔'),
        'limit': MCPParameterDefinition(type: 'integer', description: '返回的最大数量，默认为 20', defaultValue: 20),
      },
      required: ['keyword'],
    ),
...
需要先通过搜索获取书籍 ID。''',
      parameters: {'book_id': MCPParameterDefinition(type: 'integer', description: '书籍 ID')},
      required: ['book_id'],
    ),
    const MCPToolDefinition(
      name: 'get_book_count',
