...
class BackupRestoreController extends MyBaseController {
    ...
    if (await directory.exists()) {
      ...
      for (var entity in entities) {
        ...
        if (entity is Directory &&
            entity.path.contains('daily_satori_backup_')) {
  ...
  Future<bool> restoreBackup() async {
    ...
    String backupDir = SettingService.i.getSetting(SettingService.backupDirKey);
    String backupFolder =
        path.join(backupDir, 'daily_satori_backup_$backupName');
    ...
    if (await imagesFile.exists() &&
        await screenshotsFile.exists() &&
        await databaseFile.exists()) {
      logger.i("恢复备份 images 目录");
      await ZipFile.extractToDirectory(
          zipFile: imagesFile,
          destinationDir: Directory(path.join(appDocDir, 'images')));
      logger.i("恢复备份 screenshots 目录");
      await ZipFile.extractToDirectory(
          zipFile: screenshotsFile,
          destinationDir: Directory(path.join(appDocDir, 'screenshots')));
      logger.i("恢复备份数据库文件");
      await ZipFile.extractToDirectory(
          zipFile: databaseFile,
          destinationDir:
              Directory(path.join(appDocDir, ObjectboxService.dbDir)));
