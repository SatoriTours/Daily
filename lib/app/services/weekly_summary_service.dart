...
class WeeklySummaryService {
  ...
  String _buildSummaryPrompt(
    ...
    buffer.writeln();
    buffer.writeln('要求：');
    buffer.writeln('1. 用Markdown格式输出');
    buffer.writeln('2. 分析这周阅读/收藏的内容主题和趋势');
    buffer.writeln('3. 提取关键洞察和学习要点');
    buffer.writeln('4. 如果有日记，分析情绪变化和重要事件');
    buffer.writeln('5. 给出简短的周度总结和建议');
    buffer.writeln('6. 在提到具体文章或日记时，用特殊格式标注：[[article:ID:标题]] 或 [[diary:ID:日期]]');
    buffer.writeln('7. 输出结构化、美观、易读');
    buffer.writeln();
