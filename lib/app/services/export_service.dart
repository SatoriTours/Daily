...
class ExportService {
  ...
  void _addJsonFile(arch.Archive archive, String name, String jsonStr) {
    ...
    archive.addFile(arch.ArchiveFile(name, bytes.length, bytes));
  }
  ...
  Future<void> _collectAndAddImages(
    ...
    final step = collected.isNotEmpty ? 0.6 / collected.length : 0.0;
    var imageIdx = 0;
    ...
    for (final relPath in collected) {
      ...
      if (await file.exists()) {
        ...
        archive.addFile(arch.ArchiveFile(relativePath, bytes.length, bytes));
      } else {
