...
internal sealed interface DiaryRecordingResult {
    ...
    data class ForegroundEntryFinished(
        override val sessionToken: String,
...
internal class DiaryRecordingActor(
    ...
    private suspend fun retryPersistence(): DiaryRecordingCommandResult {
        ...
        requestForeground(session, failed)
        persistTerminal(session)
        return DiaryRecordingCommandResult.Accepted
    }
    ...
    private suspend fun processResult(result: DiaryRecordingResult) {
        ...
        when (result) {
            is DiaryRecordingResult.ForegroundEntryFinished -> {
                if (
                    result.errorCode != null &&
                    result.sessionCreatedAtMonotonicMs == session.createdAtMonotonicMs &&
                    host.isCurrentHostGeneration(result.hostGeneration) &&
                    session.pendingPersistence == null &&
                    !session.foregroundFailureHandled
                ) {
                    session.foregroundFailureHandled = true
                    session.captureElapsed(monotonicNowMs())
                    ticker?.cancel()
                    ticker = null
                    publish(session.failedState(result.errorCode, null))
            ...
            is DiaryRecordingResult.RecorderStarted -> {
                ...
                if (session.terminalRecorderOperation != TerminalRecorderOperation.None) {
                    Unit
            ...
            is DiaryRecordingResult.RecorderStopped -> {
                session.terminalRecorderOperation = TerminalRecorderOperation.Finished
                completeRecorderOutput(session, result.output)
            }
            is DiaryRecordingResult.RecorderStopFailed -> {
                if (session.terminalRecorderOperation == TerminalRecorderOperation.StopInFlight) {
                    session.terminalRecorderOperation = TerminalRecorderOperation.None
                    launchRelease(session, result.errorCode, decision = ReleaseDecision.Fail)
                }
            }
    ...
    private fun launchStop(session: Session) {
        if (session.terminalRecorderOperation != TerminalRecorderOperation.None) return
        session.terminalRecorderOperation = TerminalRecorderOperation.StopInFlight
        launchRecorder(session.token) {
            try {
    ...
    private fun launchRelease(
        ...
        if (session.terminalRecorderOperation != TerminalRecorderOperation.None) return
        session.terminalRecorderOperation = TerminalRecorderOperation.ReleaseInFlight
        launchRecorder(session.token) {
    ...
    private suspend fun persistTerminal(session: Session) {
        ...
        if (shutdownRequested) closeRecorderBoundary()
    }
    ...
    private fun handoffTerminal(session: Session) {
        ...
        if (shutdownRequested) closeTerminalBoundary()
    }
    ...
    private fun requestForeground(
        session: Session,
        state: DiaryRecordingState = store.state.value,
    ) {
        host.requestForeground(session.token, session.createdAtMonotonicMs, state)
    }

    private fun closeTerminalBoundary() {
    ...
    private enum class TerminalRecorderOperation {
        None,
        StopInFlight,
        ReleaseInFlight,
        Finished,
    ...
    private inner class Session(
        ...
        var terminalRecorderOperation: TerminalRecorderOperation = TerminalRecorderOperation.None,
        var foregroundFailureHandled: Boolean = false,
        ...
        fun startingState() = DiaryRecordingState.Starting(
            diaryId,
            attachmentId,
            createdAtMonotonicMs = createdAtMonotonicMs,
        )
        fun recordingState() = DiaryRecordingState.Recording(
            diaryId,
            attachmentId,
            elapsed(),
            createdAtMonotonicMs,
        )
        fun pausedState() = DiaryRecordingState.Paused(
            diaryId,
            attachmentId,
            accumulatedMs,
            createdAtMonotonicMs,
        )
        fun stoppingState() = DiaryRecordingState.Stopping(
            diaryId,
            attachmentId,
            accumulatedMs,
            createdAtMonotonicMs,
