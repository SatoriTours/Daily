...
class RepositoryQuerySimplificationSourceTest {
    ...
    fun searchRepositoriesUseFtsQueriesWithLegacyLikeFallback() {
        ...
        assertTrue(schema.contains("CREATE VIRTUAL TABLE article_fts USING fts4"))
        assertTrue(schema.contains("CREATE VIRTUAL TABLE diary_fts USING fts4"))
        assertTrue(schema.contains("CREATE VIRTUAL TABLE book_fts USING fts4"))
        assertTrue(schema.contains("CREATE VIRTUAL TABLE book_viewpoint_fts USING fts4"))
        assertTrue(schema.contains("CREATE VIRTUAL TABLE memory_entry_fts USING fts4"))
