...
private val SecretRegex = Regex("(?i)(?:\\b(?:api[_-]?key|token|secret|password)\\b|密码|密钥)\\s*[:=：是]?\\s*[A-Za-z0-9_\\-]{4,}")
private val IdCardRegex = Regex("\\b\\d{17}[0-9Xx]\\b")
private val PhoneRegex = Regex("(?<!\\d)1[3-9]\\d{9}(?!\\d)")
private val PersonRegex = Regex("(?:我叫|姓名|名字|朋友|同事|老板|同学|老师)([\\u4e00-\\u9fa5]{2,4})")
...
class PrivacyMasker {
    ...
    fun mask(text: String): String {
        var result = text
        result = maskRegex(result, "SECRET", SecretRegex)
        result = maskRegex(result, "ID", IdCardRegex)
        result = maskRegex(result, "PHONE", PhoneRegex)
        result = maskRegex(result, "PERSON", PersonRegex) { match ->
            match.groupValues[1]
        }
        return result
    }
