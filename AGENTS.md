...
- 长任务优先使用完成通知；无通知时使用工具支持的等待接口，避免高频查询。等待期间继续独立工作，单次阻塞等待不超过宿主限制。
...
- 不重复查询已确认且版本匹配的文档；Context7 的触发和重试规则见下文。
...
仅当库、框架、SDK、API、CLI 或云服务的当前 API 信息不足或需要核实时，使用 Context7 CLI 查询；已有版本匹配的本地文档或本会话查询结果可复用：

1. `npx ctx7@latest library <name> "<question>"`
2. 选择官方且最相关的 `/org/project` ID。
3. `npx ctx7@latest docs <libraryId> "<question>"`

每个问题最多一次 `library` 和一次 `docs` 调用；网络失败最多额外重试一次，沙箱导致的网络错误按宿主权限机制重试。不得在查询中包含密钥或敏感信息；配额错误时明确告知用户。
...
2. **代码质量**：函数默认 ≤50 行、缩进默认 ≤3 层；清晰的声明式 UI 可例外，不为满足行数而机械拆分。只检查本次修改，不据此扩大重构，避免重复代码。
3. **样式系统**：`import com.dailysatori.ui.theme.*`，禁止硬编码颜色/间距/字体
4. **质量检查**：按下文“代码校验”执行与本次改动相关的验证
...
代码改动完成后运行一次相关测试和必要的编译检查；纯文档修改无需构建。只有涉及打包、资源、构建配置或准备发布时才运行 `assembleDebug`，与编译检查合并为一次 Gradle 调用。同一代码和环境已有有效验证结果时直接复用；后续修改、失败或新的相关风险才触发受影响检查的重跑。
...
3. 日常使用旧版本数据库样本进行代码级迁移测试，检查数据保留及新结构可用；发布前或用户要求设备验证时，保留旧数据执行覆盖安装验证，不以干净安装代替升级验证
